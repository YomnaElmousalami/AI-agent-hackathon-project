...
![Technical Architecture Diagram](images/diagram.png)
...
## Phase 3:
Hugging face model: llama3.2, in the llm directory llama.py file
...
- To run the curriculum planner agent in the cli:
"python .\langchain\curriculum_planner_agent.py"

- To run the teacher agent in the cli:
"python -m langchain.teacher_agent"

- To run the knowledge validation agent in the cli:
"python .\langchain\knowledge_validation_agent.py"

- To run the resource recommendation agent in the cli:
"python .\langchain\resource_recommendation_agent.py"

Accident Mode (Real-Time Assistance):

- To run the accident reporting agent in the cli:
"python .\langchain\accident_reporting_agent.py"

- To run the accident severity assessment agent in the cli:
"python .\langchain\accident_severity_assesment_agent.py"

- To run the policy interpretation agent in the cli:
"python .\langchain\policy_interpretation_agent.py"

- To run the claims preparation agent in the cli:
"python .\langchain\claims_preparation_agent.py"

- To run the action plan agent in the cli:
"python .\langchain\action_plan_agent.py"

### Postman MCP testing
In postman, select http and type:

- `http://localhost:8000/mcp`

To run in powershell in order to start postman:
